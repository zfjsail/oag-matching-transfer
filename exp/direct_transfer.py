...
def evaluate(epoch, encoders, data_loader, return_best_thrs, args, thr=None):
    encoder_src, encoder_dst = encoders
    map(lambda m: m.eval(), [encoder_src, encoder_dst])
    correct = 0
    tot_cnt = 0
    y_true = []
    y_pred = []
    ...
    if args.base_model == "cnn":
        for batch1, batch2, label in data_loader:
            ...
            output, cur_hidden = encoder_src(batch1, batch2, rsv_layer=args.n_tune)
            ...
            loss_batch = F.nll_loss(output, label)
            # loss_batch = F.nll_loss(torch.log(oup∂ut), label)
        ...
        for batch1, batch2, batch3, batch4, label in data_loader:
            ...
            output, cur_hidden = encoder_src(batch1, batch2, batch3, batch4, rsv_layer=args.n_tune)
            # cur_output = classifier(cur_hidden)
...
def train(args):
    model_dir = os.path.join(settings.OUT_DIR, args.test)
    wf = open(os.path.join(model_dir, "{}_{}_direct_transfer_results.txt".format(
        args.test, args.base_model)), "w")
